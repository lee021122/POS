...
*/
	...
	IF NOT EXISTS (
		SELECT 1
		FROM tb_product a
		WHERE a.product_id = p_link_item
	) THEN
...
-- 	END LOOP;
	...
	IF NOT EXISTS (
		...
		INSERT INTO tb_modifier_item_link (
			...
			gen_random_uuid(), v_now, p_current_uid, v_now, p_current_uid, p_modifier_group_id, p_link_item
		);

		audit_log := 'Added product link: ' || p_link_item || '.';
		
	ELSE 
	
		DELETE FROM tb_modifier_item_link WHERE modifier_group_id = p_modifier_group_id AND product_id = p_link_item;
		...
		audit_log := 'Updated product link: ' || p_link_item || '.';
